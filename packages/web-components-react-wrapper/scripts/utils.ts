...
export const toModuleFile = (
  ...
  elementName: string,
  customEvents: string[] = [],
) => getComponentFile('component.ts', { elementName, defineCustomElementFunction, importPath, customEvents });

export const toServerOnlyModuleFile = (elementName: string) => getComponentFile('component.server-only.ts', { elementName });

const toElementTypeDeclaration = (
  elementName: string,
  { name, events }: { name: string; events: string[] } = { name: '', events: [] },
) =>
  getComponentFile('types.component.d.ts', {
    elementName,
